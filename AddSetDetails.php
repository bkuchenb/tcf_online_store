...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['setInfo']))
{
	...
	for($i = 0; $i < count($array); $i++)
	{
		...
		if(count($temp) > 1)//look for the line with the card information
		{
			//add the quantity
			$cardData[$newIndex]['qty'] = $temp[0];
			//add the year
			$year = $temp[1];
			
			//add the card information to the array
			$splicedData = spliceCard($temp);
			//$set = str_replace(" ", "_", $splicedData[0]);
			$set = $splicedData[0];
			$cardData[$newIndex]['cardNumber'] = $splicedData[1];
			$cardData[$newIndex]['cardName'] = $splicedData[2];
			$cardData[$newIndex]['high'] = $splicedData[3];
			$cardData[$newIndex]['low'] = $splicedData[4];
	...
	echo'
	<div class="infoTable">
		<form method="post" action="AddSetDetails.php">
...
else
{
	...
	echo'
	<p>
	<form method="post" action="addSetDetails.php" align="center">
...
function spliceCard($array)
{
	...
	//save the card name
	$cardName = "";
	for($y = $index + 1; $y < count($array) - 4; $y++)
	{
		$cardName = $cardName . $array[$y] . " ";
	}
	$cardName = trim($cardName);
	//save the high price
	$high = str_replace("\$", "", $array[count($array) - 4]);
	//save the low price
	$low = str_replace("\$", "", $array[count($array) - 3]);
...
function updateDatabase($sport, $year, $set, $cardData)
{
	...
	if ($r)//if the create table query ran ok display a message and insert the card info
	{
		...
		$q = 'INSERT INTO ' . $sport . '_' . $id . '(set_id, quantity, card_number, name, price_1, price_2)
			VALUES ';
		...
		for($j = 0; $j < count($cardData); $j++)//add to the sql statement
		{
			...
			//add to the query
			$q = $q . "'" . $id . "', '" . $cardData[$j]['qty'] . "', '" . addslashes($cardData[$j]['cardNumber']) .
